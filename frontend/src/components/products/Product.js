    ...
    return (
        <div className="flex flex-col justify-center items-center space-y-3 bg-white border border-gray-200 hover:shadow-xl transition duration-700 ease-in-out transform hover:scale-105 p-4 box-border rounded-xl">
            ...
            <div className="flex items-center space-x-3">
                {user.displayName && (
                    <>
                        <button disabled={disabled} className={` ${disabled} && "opacity-30" w-36 btn-primary py-3 px-4 poppins text-sm flex items-center justify-center space-x-3 text-center bg-green-500`} onClick={() => {
                            handleCart(props);
                            setDisabled(true)
                ...
                <Button className="w-36 btn-primary bg-green-500 py-3 px-2 poppins text-sm hover:bg-yellow-300" text="View" onClick={() => history.push(`/products/${title}`)} />
            </div>
